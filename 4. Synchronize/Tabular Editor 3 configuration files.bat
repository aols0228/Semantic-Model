...
echo Sletter og kopierer Tabular Editor 3 konfigurationsfiler via GitHub:
echo.
...
set "currentDir=%~dp0"
REM Use the for loop to set the parent directory with a backslash in the end
for %%A in ("%currentDir%..\") do set "parentDir=%%~dpA"
REM Determine the DAP model configuration files directory
set "ModelAppDataDir=%parentDir%Model\Configuration Files\"
...
REM call "%parentDir%5. Admin\Git-scripts - Pull.bat" "%parentDir%" "%currentDir%"
